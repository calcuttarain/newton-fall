...
class Memory():
    ...
    def sample(self, batch_size):
        ...
        for index in indices:
            ...
            state = np.moveaxis(state, 0, 2) / 255.0  # Normalize and rearrange dimensions
...
def initialize_new_game(RLGame, agent):
    """
    Initialize a new game by resetting the game and storing initial dummy experiences.
    """
    RLGame.restart()
    starting_frame = resize_frame(RLGame.getLastFrame())
...
def take_step(RLGame, agent, score, debug=False, training=True):
    """
    ...
    RLGame.step(agent.memory.actions[-1])

    next_frame = RLGame.getLastFrame()
    health = RLGame.getHealth()
    reward = -200 + health
    done = RLGame.isGameOver()
    ...
    new_state = torch.tensor(new_state).unsqueeze(0).permute(0, 3, 1, 2).to(agent.device)  # PyTorch format
...
def play_episode(RLGame, agent, debug=False, training=True):
    """
    Play an episode and return the final score.
    """
    initialize_new_game(RLGame, agent)
    done = False
    score = 0
    while not done:
        score, done = take_step(RLGame, agent, score, debug, training)
...
def train_agent(agent, selected_level, experiment_name):
    ...
    if not levels:
        ...
        return
    # selected_level = 4
    RlGame = make_RLGame(levels[selected_level])
    last_100_avg = [0]
    ...
    for i in range(102):
        ...
        timee = time.time()
        score = play_episode(RlGame, agent, debug = True, training=True) #set debug to true for rendering
        scores.append(score)
        if score > max_score:
            max_score = score
...
agent = Agent(possible_actions=[0,1,2],starting_mem_len=5000,max_mem_len=750000,starting_epsilon=1,learn_rate=.0005)
